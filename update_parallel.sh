...
LOG_DIR="${HOME}/out"
LOG_FILE="${LOG_DIR}/update_parallel-$(date -I).out"
...
for DEVICE in ${DEVICES[@]}; do 
	...
	if [[ "${SANITY,,}" == "true" ]]; then
		if [[ "${DEVICE_VERSION}" == "${NEW_VERSION}" ]]; then
			...
			echo "DEVICE at $DEVICE_VERSION, want to be at $NEW_VERSION" >> ${LOG_FILE}
		fi
	fi
	cd $HOME/esphome/
	
	NOTIFY=true
	while [[ "$(screen -ls | grep -c yaml)" -ge "${PARALLEL}" ]]; do
		if [[ "${NOTIFY}" == "true" ]]; then
			echo "Waiting to start ${DEVICE}"
			NOTIFY="false"
		fi
		sleep 3
	done
	echo screen -S "$(basename ${DEVICE})" -d -m ./sub_proc "${DEVICE}" "${LOG_FILE}"
	screen -S "$(basename ${DEVICE})" -d -m ./sub_proc "${DEVICE}" "${LOG_FILE}"
done
echo "Ending at $(date)" >> ${LOG_FILE}
